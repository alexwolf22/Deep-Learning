function [iscorrect, err] = recurrent_lstm_test()
...
netspec_opts.layers(2).subtype = 'recurrent_lstm';
...
for i = 1:numel(dzdcell_t)
  res{1} = rnn_init_forward_states(net, bs);
  res{1}(2).cell(i) = res{1}(2).cell(i) + DELTA;
  res = forward(net, x,  y,bs, ts, res);
  dzdcell0 = sum(cellfun(@(c) c(end).x, res));

  res{1} = rnn_init_forward_states(net, bs);
  res{1}(2).cell(i) = res{1}(2).cell(i) - DELTA;
  res = forward(net, x, y, bs, ts, res);
  dzdcell1 = sum(cellfun(@(c) c(end).x, res));
  dzdcell_n(:, i) = (dzdcell0(:) -  dzdcell1(:)) ./ (2*DELTA);
end
...
dzdcell_n = reshape(sum(dzdcell_n, 1), size(dzdcell_t));
...
[c, e] = test(dzdcell_n, dzdcell_t, TOL);
iscorrect = iscorrect && c;
err.dzdcell = e;
